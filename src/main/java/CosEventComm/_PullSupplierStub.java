...
public class _PullSupplierStub
	...
	implements CosEventComm.PullSupplier
{
	/** Serial version UID. */
	...
	public final static java.lang.Class _opsClass = CosEventComm.PullSupplierOperations.class;
		...
		while(true)
		{
			if(! this._is_local())
			{
				...
				catch( org.omg.CORBA.portable.ApplicationException _ax )
				{
					...
					try
					{
						if( _id.equals("IDL:CosEventComm/Disconnected:1.0"))
						{
							throw CosEventComm.DisconnectedHelper.read(_ax.getInputStream());
						}
						else 
						{
							throw new RuntimeException("Unexpected exception " + _id );
